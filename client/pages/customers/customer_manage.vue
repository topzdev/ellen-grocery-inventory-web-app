<template>
  <v-form ref="manageForm" v-model="valid" lazy-validation>
    <v-card flat>
      <v-card-actions>
        <back-btn to="/others" title="Manage Customer" />
      </v-card-actions>
...
export default class AddCustomer extends CustomerMixin {
  created() {
    let show;
    let customer = this.customerStore.getCurrentCustomer;
  ...
  destroyed() {
    this.isEdit = false;
    this.customerStore.fetchSingleCustomer(undefined);
  }
}
</script>
